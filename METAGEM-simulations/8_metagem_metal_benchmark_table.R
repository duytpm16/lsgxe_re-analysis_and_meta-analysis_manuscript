...
# 1 - exposure
mb_metal_process <- fread("mb_metal_process_resource_usage.log", fill = T)
mb_metal_system  <- fread("mb_metal_system_resource_usage.log",  skip = 1)
rb_metal_process <- fread("rb_metal_process_resource_usage.log", fill = T)
rb_metal_system  <- fread("rb_metal_system_resource_usage.log",  skip = 1)

mb_metagem_process <- fread("mb_metagem_process_resource_usage.log", fill = T)
mb_metagem_system  <- fread("mb_metagem_system_resource_usage.log",  skip = 1)
rb_metagem_process <- fread("rb_metagem_process_resource_usage.log", fill = T)
...
metagem_memory <- max(c(mb_metagem_process$V12, rb_metagem_process$V12), na.rm = T) / 1000000


# 2 - exposures
mb_metagem_process <- fread("mb_metagem_2e_process_resource_usage.log", fill = T)
mb_metagem_system  <- fread("mb_metagem_2e_system_resource_usage.log",  skip = 1)
rb_metagem_process <- fread("rb_metagem_2e_process_resource_usage.log", fill = T)
...
metagem_2e_memory <- max(c(mb_metagem_process$V12, rb_metagem_process$V12), na.rm = T) / 1000000
