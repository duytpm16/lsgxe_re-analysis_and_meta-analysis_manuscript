...
dx mkdir "/METAGEM-METAL-Comparisons/1-Exposure/GEM/combined"

for j in "${sets[@]}"
do
	for i in {1..22} # Chromosomes
	do
		dx cd "/METAGEM-METAL-Comparisons/1-Exposure/GEM/${j}/Chr${i}"
		dx mv "gem_res" "gem_${j}_chr${i}.out"
		dx download "gem_${j}_chr${i}.out"

		if [[ $i -gt 1 ]]
		then
			tail -n +2 "gem_${j}_chr${i}.out" >> "gem_${j}_combined.out"
		else
