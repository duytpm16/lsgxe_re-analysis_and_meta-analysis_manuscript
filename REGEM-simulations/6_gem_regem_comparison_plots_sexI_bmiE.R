...
gem <- as.data.frame(fread("gem_sexI_bmiE_combined.out"))
...
gc()
regem <- as.data.frame(fread("regem_sexI_bmiE_results"))
...
intnames   <- c("G", "G-BMI", "G-sex")
titlenames <- c("Main", "G x BMI", "G x Sex")
combo      <- c("G_G-BMI", "G_G-sex", "G-BMI_G-sex")
combotitle <- c("Main - G x BMI", "Main - G x Sex", "G x BMI - G x Sex")
...
for (j in 1:3) {
  ...
  if (j == 1) {
    ...
    p <- p +
      scale_y_continuous(breaks = c(0, 0.005, 0.01, 0.015), limits = c(0, 0.015)) +
      scale_x_continuous(breaks = c(0, 0.005, 0.01, 0.015), limits = c(0, 0.015))
  } else if (j == 3) {
    p <- p +
