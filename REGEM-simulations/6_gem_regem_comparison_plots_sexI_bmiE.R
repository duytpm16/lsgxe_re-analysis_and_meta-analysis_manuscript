...
combo <- c("G_G-sex", "G_G-BMI", "G-BMI_G-sex")
combotitle <- c("G - G x BMI", "G - G x sex", "G x BMI - G x sex")
index = 3
for (j in 1:3) {
...
for (j in 1:3) {
  ...
  if (j == 1) {
    breaks = c(-2.5e-05, 0, 2.5e-05, 7e-05)
  } else if (j == 2) {
    breaks = c(-6.5e-04, -3e-04, 0, 3.5e-04)
  } else {
    breaks = c(-2e-04, -1e-04, 0, 5e-05)
  }
